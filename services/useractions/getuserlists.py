...
get_user_lists_api = Blueprint('get_user_lists_api', __name__)

@get_user_lists_api.route('/user/<userid>/lists', methods=['GET'])
def get_user_lists(userid= None):
    ...
    try:
        if userid is not None:
            ...
            reponse_obj.Data = jsonpickle.decode(dumps({'shortlist': user['shortlist'], 'applylist': user['applylist']}))
